      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-14">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[10px] bg-[#E94335]/10 text-[#E94335] font-semibold px-2 py-0.5 rounded">Admin</span>
          </div>
          <div className="flex items-center gap-3 text-sm">
            <span className="text-[#757575] hidden sm:inline">{user.email}</span>
            <a href="/dashboard" className="text-[#4285F4] hover:underline">Dashboard</a>
            <button onClick={async () => { await logout(); router.push("/"); }} className="text-[#E94335] hover:underline">Sign out</button>
          </div>
        </div>
        <div className="border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-4 flex gap-5 overflow-x-auto">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href}
                className={`text-xs font-medium py-3 border-b-2 transition-colors whitespace-nowrap ${
                  pathname === link.href ? "border-[#4285F4] text-[#4285F4]" : "border-transparent text-[#757575] hover:text-[#202124]"
                }`}>{link.label}</a>
            ))}
          </div>
        </div>
